module.exports = {
  siteMetadata: {
    title: 'Sagar Kharabe',
    subtitle: 'FullStack Engineer',
    description: `Hi, I'm Sagar Kharabe, I'm passionate about coding.`,
    siteUrl: 'https://sagarkharabe.com',
    disqusShortname: 'sagarkharabe',
    githubUser: 'sagarkharbe',
    social: [
      { title: 'Github', link: 'https://github.com/sagarkharbe', icon: 'github' },
      { title: 'Twitter', link: 'https://twitter.com/sagarkharabe', icon: 'twitter' },
      { title: 'Linkedin', link: 'https://linkedin.com/in/sagarkharabe', icon: 'linkedin' },
      { title: 'Medium', link: 'https://medium.com/@sagarkharabe', icon: 'medium' },
      { title: 'Dev', link: 'https://instagram.com/sagar.r.k', icon: 'dev' },
      {
        title: 'StackOverflow',
        link: 'https://instagram.com/sagar.r.k',
        icon: 'stack-overflow',
      },
      // { title: 'Goodreads', link: 'https://instagram.com/dantehemerson', icon: 'instagram' },
      {
        title: 'FreeCodeCamp',
        link: 'https://instagram.com/sagarkharabe',
        icon: 'free-code-camp',
      },
      { title: 'Gitlab', link: 'https://instagram.com/sagarkharbe', icon: 'gitlab' },
      { title: 'Angellist', link: 'https://angel.co/sagarkharabe', icon: 'angellist' },
      { title: 'Codepen', link: 'https://codepen.io/sagarkharabe', icon: 'codepen' },
      // { title: 'Pinterest', link: 'https://www.pinterest.com/dantehemerson/', icon: 'pinterest' },
      // { title: 'Steam', link: 'https://steamcommunity.com/id/dantehemerson/', icon: 'steam' },
      // { title: 'Twitch', link: 'https://www.twitch.tv/dantehemerson', icon: 'twitch' },
      {
        title: 'Youtube',
        link: 'https://www.youtube.com/channel/UCpQvhmrcJQHD2LR8Yj6-Glg',
        icon: 'youtube-play',
      },
    ],
    socials: {
      github: 'https://github.com/sagarkharbe',
      twitter: 'https://twitter.com/sagarkharabe',
      linkedin: 'https://linkedin.com/in/sagarkharabe',
      instagram: 'https://instagram.com/sagar.r.k',
    },
    menu: [
      { title: 'Home', id: '', to: '/', icon: 'home' },
      { title: 'Blog', id: 'Blog', to: '/blog', icon: 'home' },
      { title: 'About', id: 'About', to: '/about', icon: 'home' },
      { title: 'Portfolio', id: 'Portfolio', to: '/portfolio', icon: 'home' },
      //{ title: 'Contact', id: 'Contact', to: '/contact', icon: 'home' }
    ],
  },
}
